...
class data_preprocess_ATT_bert_nfold():
    ...
    #begin
    def __init__(self,sentences,images,mids,y,config,pathset,caption_tokens_ids): #sentence==token_ids
    #end
        ...
        #begin
        self.caption_tokens_ids = caption_tokens_ids
        #end
    ...
    def get_transe(self):
        ...
        vec = np.memmap(self.transE_path, dtype='float32', mode='r', shape=(4034700,))
        
        self.embedding = vec.reshape((-1,50))
        #self.embedding = vec.reshape((-1,5))
    ...
    def load_data(self):
        ...
        caption_input_ids = []
        for cap_tkn in self.caption_tokens_ids:
            caption_input_ids.append(cap_tkn['input_ids'].squeeze(0))
            caption_attention_masks.append(cap_tkn['attention_mask'].squeeze(0))
        #end
        ...
        #begin
        names_dict = {'input_ids':input_ids,'attention_masks':attention_masks,'image':self.images,\
        'vec_1':vec_all_1, 'vec_2':vec_all_2, 'sim_list':sim_all,'y':y, 'caption_attention_masks':caption_attention_masks,'caption_input_ids':caption_input_ids}
        #end
