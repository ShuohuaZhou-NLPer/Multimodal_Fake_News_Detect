...
def train(train_dict,val_dict,test_dict,model_mode,device,config,dataset,args,transform,pathset,p):
    ...
    #begin
    print ('--------caption_input_ids-----------',len(train_dict['caption_input_ids']))
    print ('-------------caption_attention_masks-----------',len(train_dict['caption_attention_masks']))
    print ('------------inuputids------------',len(train_dict['input_ids']))
    train_x_input_ids, train_x_attention_masks, train_x_img,\
    train_x_kg_1, train_x_kg_2, train_x_kg_sim,\
    train_y,train_caption_attention_masks,train_caption_input_ids  = train_dict['input_ids'],train_dict['attention_masks'],train_dict['image'],\
                          train_dict['vec_1'],train_dict['vec_2'],train_dict['sim_list'],\
                          train_dict['y'],train_dict['caption_attention_masks'],train_dict['caption_input_ids']

    val_x_input_ids, val_x_attention_masks, val_x_img, \
    val_x_kg_1, val_x_kg_2, val_x_kg_sim, \
    val_y,val_caption_attention_masks,val_caption_input_ids = val_dict['input_ids'], val_dict['attention_masks'], val_dict['image'], \
                           val_dict['vec_1'], val_dict['vec_2'], val_dict['sim_list'], \
                           val_dict['y'],val_dict['caption_attention_masks'],val_dict['caption_input_ids']
    ...
    train_dataset = Dataset_all(x_txt_input_ids=train_x_input_ids, \
                                     ...
                                     x_caption_attention_masks=train_caption_attention_masks, x_caption_input_ids=train_caption_input_ids)
    ...
    val_dataset = Dataset_all(x_txt_input_ids=val_x_input_ids, \
                                   ...
                                   x_caption_attention_masks=val_caption_attention_masks, x_caption_input_ids=val_caption_input_ids)
    #end
    ...
    bert_params = list(map(id,model.txtenc.bert.parameters()))
    ...
    optimizier = optim.Adam([
        ...
        {'params':model.txtenc.bert.parameters(),'lr':5e-5},
        {'params':base_params}
    ],lr = args.lr)
    ...
    for epoch in range(args.epoch):
        ...
        #begin
        for i,(input_ids, attention_masks, img, kg1, kg2, kg_sim, labels,cap_mask,cap_ids) in enumerate(train_loader):
            cap_mask=cap_mask.to(device, dtype=torch.long)
            cap_ids=cap_ids.to(device, dtype=torch.long)
        #end
            ...
            #begin
            outputs_class = model(input_ids,attention_masks,img,kg1,kg2,kg_sim,args,cap_ids,cap_mask)
            #end
            #outputs_class = model(input_ids,attention_masks,img,kg1,kg2,kg_sim,args)
            print ('outputs_class--------------',outputs_class,outputs_class.shape)
            #outputs_class = outputs_class.squeeze()
            outputs_class = outputs_class.squeeze(1)
            print ('outputs_class-2--------------',outputs_class,outputs_class.shape)
            print ('labels-2-----------',labels,labels.shape)
            loss_class = criterion_clf(outputs_class,labels)
            loss = loss_class + args.lambda_orth*criterion_orth(p_img=model.ln_uniq_img.weight,\
                                                   p_txt=model.ln_uniq_txt.weight,\
                                                   w_shr=model.ln_shr.weight)
            loss.backward()
            optimizier.step()
        ...
        with torch.no_grad():
            ...
            #begin
            for i, (input_ids, attention_masks, img, kg1, kg2, kg_sim, labels,cap_mask,cap_ids) in enumerate(val_loader):
                cap_mask=cap_mask.to(device, dtype=torch.long)
                cap_ids=cap_ids.to(device, dtype=torch.long)    
            #end
                input_ids = input_ids.to(device, dtype=torch.long)
...
def test(test_dict,model,device,config,dataset,model_mode,args,transform,pathset,p):
    ...
    test_y,test_caption_attention_masks,test_caption_input_ids = test_dict['input_ids'], test_dict['attention_masks'], test_dict['image'], \
                       test_dict['vec_1'], test_dict['vec_2'], test_dict['sim_list'], \
                       test_dict['y'],test_dict['caption_attention_masks'],test_dict['caption_input_ids']
    ...
    test_dataset = Dataset_all(x_txt_input_ids=test_x_input_ids, \
                                          ...
                                          pathset=pathset,\
                                          x_caption_attention_masks=test_caption_attention_masks, x_caption_input_ids=test_caption_input_ids)
    #end
    ...
    with torch.no_grad():
        ...
        #begin
        for i, (input_ids, attention_masks, img, kg1, kg2, kg_sim, labels,cap_mask,cap_ids) in enumerate(test_loader):
            cap_mask=cap_mask.to(device, dtype=torch.long)
            cap_ids=cap_ids.to(device, dtype=torch.long)        
        #end
            input_ids = input_ids.to(device, dtype=torch.long)
...
def main(args):
    ...
    #begin
    X_txt, X_img, X_kg, y ,caption_tokens_ids= pre_training_pheme(pathset, config)
    #end
    ...
    #begin
    preprocess = data_preprocess_ATT_bert_nfold(X_txt, X_img,X_kg,y,config,pathset, caption_tokens_ids)
    #end
